...
public interface IRepository
{
	...
	/// collection of <see cref="RoleOption"/> entities.</returns>
	public Task<IEnumerable<RoleOption>> GetAll(int limit, int offset, Expression<Func<RoleOption, bool>>? filter = null);

	/// <summary>
	/// Gets a single <see cref="RoleOption"/> entity from the database that meets the
	/// specified filter.
	/// </summary>
	/// <param name="filter">The lambda expression to filter the entity.</param>
	/// <returns>A task that represents the asynchronous operation and returns the
	/// <see cref="RoleOption"/> entity, or <c>null</c> if none is found.</returns>
	...
	/// <returns>A task that represents the asynchronous operation and returns the
	/// updated <see cref="RoleOption"/> entity.</returns>

...
public class Service(ILogger<IService> logger, IRepository repository, IMapper mapper) : IService
{
	...
	public async Task<RoleDto?> Update(RoleUpdateDto roleUpdateDto)
	{
		...
		try
		{
			RoleDto? existingRole = await GetOne(role => role.Id == roleUpdateDto.Id);
			...
			if (!existingRole.RowVersion.SequenceEqual(roleUpdateDto.RowVersion))
			{
	...
	public async Task<bool> Delete(int id)
	{
		...
		try
		{
			Role role = await _repository.GetOne(role => role.Id == id) ?? throw new Exception("NULL");

            role.Status = EntityStatus.Inactive;
			role.UpdatedAt = DateTime.UtcNow;
			role.RoleOptions.Clear();

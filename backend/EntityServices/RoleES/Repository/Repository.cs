...
public class Repository : IRepository
{
	...
	public async Task<Role?> Update(Role role)
	{
		...
		try
		{
			...
			if (!existingRole.RowVersion.SequenceEqual(role.RowVersion))
			{
			...
			ICollection<RoleOption> newRoleOptions = await _context.RoleOptions
				.Where(roleOption => role.RoleOptions.Select(r => r.Id).Contains(roleOption.Id))
				.ToListAsync();

			existingRole.RoleOptions = newRoleOptions;
